...
|---|---|---|
| **False Positive Rate** | Proportion of legitimate alerts incorrectly flagged as suspicious | FP / (FP + TN) |
| **False Negative Rate** | Proportion of genuine threats missed by the system | FN / (FN + TP) |
...
Following commercial deployment, the following monitoring protocol
will be applied in accordance with NIST AI RMF MANAGE requirements:
...
python tests/validation/bias_test.py \
  --model v1.2 \
  --dimensions geography,size,entity_type,sector \
  --output results/v1.2/bias/

# Run drift detection
python tests/validation/drift_monitor.py \
  --baseline v1.2 \
  --current live \
  --psi-threshold 0.2
```
