...
|---|---|---|
| Novel adversarial techniques | Sophisticated actors may develop evasion techniques not in training data | Continuous retraining; quarterly red-team exercises planned |
| Dark web coverage gaps | Not all hidden services are accessible | Multiple crawler nodes; diverse entry point rotation |
| Synthetic training data | Models trained on synthetic data may miss real-world edge cases | Pilot programme outcomes will feed real-world retraining |
| Model drift | Financial crime patterns evolve; performance may degrade over time | Weekly drift monitoring; 30-day recalibration cycles |
| False negatives | 4.7% miss rate means some threats may not be flagged | Human analyst review of borderline cases; ensemble model design |
